...
4. Установите переменные окружения в разделе **Environment Variables**:
   - `NEXT_PUBLIC_API_URL` - URL вашего Railway бекенда (например: `https://your-backend.railway.app/api`)
     - **Важно:** Добавьте `/api` в конце URL, если ваш бекенд использует префикс `/api`
...
- Убедитесь, что `NEXT_PUBLIC_API_URL` содержит правильный URL с протоколом `https://`
- Проверьте, что бекенд доступен из интернета
